...
dash_data = {
  ...
  // B. Summary Data (Aggregate Level) - Enhanced with RMSE
  let summary = base
    .groupby(group_by)
    .rollup({
      Stations: op.count(),
...
// 6. HELPER: PERCENT ERROR BOUNDS
// Creates stepped boundary lines similar to reference document
error_bounds = {
  const max_k = dash_data.scatter.rollup({ max: op.max("Obs_K") }).get("max") || 1;

  // Define control thresholds (in thousands) and their error bounds
  const bounds = [
    ...
    { x: Math.max(max_k, 300), y_pos: 0.10, y_neg: -0.10 }
  ];
...
Plot.plot({
  ...
  marks: [
    ...
    // Error Bounds (Stepped Lines)
    // Positive bounds
    Plot.line(error_bounds, {
      x: "x", y: "y_pos",
      stroke: "gray", strokeWidth: 2, strokeOpacity: 0.4
    }),
    // Negative bounds
    Plot.line(error_bounds, {
      x: "x", y: "y_neg",
...
Plot.plot({
  marginLeft: 80,
  height: Math.max(350, dash_data.summary.numRows() * 40),
  color: {
